...
import { X, Upload, Calendar, DollarSign, Tag, Building2, FileText } from 'lucide-react';
import { expenseService, type CreateExpenseData } from '../../services/expenses';
...
const AddExpenseForm: React.FC<AddExpenseFormProps> = ({ onSuccess, onClose }) => {
  ...
  const handleInputChange = (field: keyof CreateExpenseData, value: string | number | string[] | undefined) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (receiptFile && newExpense.id) {
        try {
          await expenseService.uploadReceipt(newExpense.id, receiptFile);
        } catch (receiptError) {

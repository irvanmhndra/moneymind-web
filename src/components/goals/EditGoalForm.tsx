...
import { goalService, type Goal, type UpdateGoalData } from '../../services/goals';
...
const EditGoalForm: React.FC<EditGoalFormProps> = ({ goal, onSuccess, onClose }) => {
  ...
  const handleInputChange = (field: keyof UpdateGoalData, value: string | number | undefined) => {
  ...
  const calculateMonthlyContribution = () => {
    ...
    const remainingAmount = formData.target_amount - goal.current_amount;

    return Math.max(0, remainingAmount / monthsLeft).toFixed(2);
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.target_amount && goal.current_amount > formData.target_amount) {
      newErrors.target_amount = 'Target amount cannot be less than current amount';
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
            <div className="flex items-center justify-between text-sm text-blue-700 mb-2">
              <span>Current progress: ${goal.current_amount.toFixed(2)}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              ...
              <p className="mt-1 text-xs text-gray-500">
                Minimum: ${goal.current_amount.toFixed(2)} (current amount)
              </p>
            </div>
          ...
          {formData.target_amount && formData.target_date && goal.current_amount < formData.target_amount && (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => handleInputChange('status', e.target.value as 'active' | 'completed' | 'paused' | 'cancelled')}
              className="input"
            >
              <option value="active">Active</option>
              <option value="paused">Paused</option>
              <option value="cancelled">Cancelled</option>
              {goal.current_amount >= (formData.target_amount || goal.target_amount) && (
